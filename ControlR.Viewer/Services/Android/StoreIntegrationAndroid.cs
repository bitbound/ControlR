...
internal class StoreIntegrationAndroid(
  ILogger<StoreIntegrationAndroid> logger) : IStoreIntegration
{
  private const int UpdateRequestCode = 89345;
  ...
  public async Task<Result> InstallCurrentVersion()
  {
    try
    {
      ...
      if (context is null)
      {
        return Result.Fail("CurrentActivity is null when checking for store updates.").Log(logger);
      }
      ...
      {
        return Result.Fail("Unexpected result when installing current version.").Log(logger);
      }
      ...
      if (!startSucceeded)
      {
        return Result.Fail("Failed to start update flow.").Log(logger);
      }
  ...
  public async Task<Result<bool>> IsUpdateAvailable()
  {
    try
    {
      ...
      if (context is null)
      {
        return Result.Fail<bool>("CurrentActivity is null when checking for store updates.").Log(logger);
      }
      ...
      if (getResult is not AppUpdateInfo info)
      {
        return Result.Fail<bool>("Unexpected result when checking for store updates.").Log(logger);
      }

      logger.LogInformation("Available version code: {VersionCode}", info.AvailableVersionCode());
    ...
    {
      return Result.Fail<bool>(ex, "Unable to check for store updates on side-loaded app.").Log(logger);
    }
    catch (Exception ex)
    {
      return Result.Fail<bool>(ex, "Error while checking for store updates.").Log(logger);
    }
  }
}
#endif