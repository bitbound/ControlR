...
- Interactive sign-in in `ControlR.ApiClient` now clears a personal access token or service account key that was already configured on the session. Sign-in previously left the configured credential in place, and because a personal access token takes header precedence it kept every request authenticating as the old principal even though the session reported itself signed in as the user. The viewer's SignalR and WebSocket connections were also established without a token, because bearer-token retrieval short-circuited on the leftover personal access token. Only one credential is ever active now.
