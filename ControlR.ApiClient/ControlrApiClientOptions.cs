...
public class ControlrApiClientOptions
{
  ...
  /// <summary>
  /// The name of the HTTP header used to send a service account credential for authentication with the ControlR API.
  /// </summary>
  public const string ServiceAccountApiKeyHeader = "x-api-key";
  ...
  /// <summary>
  /// If supplied, the client will include the service account credential in the "x-api-key" header of each request.
  /// The value is the composite key returned once by the create-credential endpoint, formatted as
  /// <c>{credentialIdHex}:{secret}</c>. A service account is provisioned by an administrator rather than
  /// generated by a user in the ControlR UI, so it authenticates as the service account's own principal.
  /// </summary>
  [ProtectedDataClassification]
  public string? ServiceAccountApiKey { get; set; }

  public override string ToString() =>
    $"BaseUrl: {BaseUrl}, PersonalAccessToken: [REDACTED], ServiceAccountApiKey: [REDACTED]";
}
