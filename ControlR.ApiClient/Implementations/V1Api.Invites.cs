...
{
  async Task<ApiResult<InviteDtos.InviteResponseDto>> IInvitesApi.CreateInvite(Guid tenantId, InviteDtos.CreateInviteRequestDto request, CancellationToken cancellationToken)
  {
    return await _client.ExecuteApiCall(async () =>
    {
      ...
      return await response.Content.ReadFromJsonAsync<InviteDtos.InviteResponseDto>(cancellationToken);
    });
  }
  ...
  async Task<ApiResult<InviteDtos.InvitesResponseDto>> IInvitesApi.GetInvites(Guid tenantId, CancellationToken cancellationToken)
  {
    return await _client.ExecuteApiCall(async () =>
      await _client.HttpClient.GetFromJsonAsync<InviteDtos.InvitesResponseDto>(
