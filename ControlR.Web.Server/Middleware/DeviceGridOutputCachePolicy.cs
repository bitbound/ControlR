...
public class DeviceGridOutputCachePolicy : IOutputCachePolicy
{
    ...
    public ValueTask CacheRequestAsync(OutputCacheContext context, CancellationToken cancellationToken)
    {
        ...
        {
            // Set cache duration
            context.ResponseExpirationTimeSpan = _cacheDuration;            // Vary by user ID
            ...
            var requestBody = reader.ReadToEndAsync(cancellationToken).GetAwaiter().GetResult();            context.HttpContext.Request.Body.Position = 0;            // Create a hash of the request body to vary by
    ...
    private string ComputeRequestHash(string requestBody)
    {
        try
        {
            ...
            return Convert.ToBase64String(hash).Substring(0, 10).Replace('/', '_').Replace('+', '-');
        }
        catch
        {
