...
internal class UiSessionProviderMac(
  IIpcServerStore ipcStore,
  IProcessManager processManager,
  ILogger<UiSessionProviderMac> logger) : IUiSessionProvider
{
  private readonly IIpcServerStore _ipcStore = ipcStore;
  private readonly IProcessManager _processManager = processManager;
  private readonly ILogger<UiSessionProviderMac> _logger = logger;
  ...
  public async Task<DeviceUiSession[]> GetActiveDesktopClients()
  {
    ...
    foreach (var server in _ipcStore.Servers)
    {
      var uiSession = new DeviceUiSession()
      {
        ...
        Type = UiSessionType.Console // On macOS, we typically have console sessions
      };
  ...
  private async Task AddVncSessions(List<DeviceUiSession> sessions)
  {
    try
    {
      ...
      foreach (var line in vncLines)
      {
            ...
            line.Contains("screensharing", StringComparison.OrdinalIgnoreCase))
        {
          ...
          var session = new DeviceUiSession
          {
            ...
            Type = UiSessionType.Console, // Treat VNC as console since it's the desktop
            ProcessId = 0
          };
