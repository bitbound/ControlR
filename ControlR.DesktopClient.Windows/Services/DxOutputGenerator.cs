...
internal interface IDxOutputGenerator : IDisposable
{
  DxOutput? GetDxOutput(string deviceName);
  void RefreshOutput();
}

internal class DxOutputGenerator(ILogger<DxOutputGenerator> logger) : IDxOutputGenerator
{
  private readonly MemoryCache _faultedDevices = new(new MemoryCacheOptions());
  private readonly ILogger<DxOutputGenerator> _logger = logger;
