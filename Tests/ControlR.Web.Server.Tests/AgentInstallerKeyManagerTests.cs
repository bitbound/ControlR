...
public class AgentInstallerKeyManagerTests(ITestOutputHelper testOutput) : IAsyncLifetime
{
  ...
  public async Task IncrementUsage_WhenTimeBasedKeyExpired_RemovesKey()
  {
    ...
    var validateResult = await _keyManager.ValidateKey(dto.KeySecret, dto.Id);
  ...
  [Fact]
  public async Task ValidateKey_RecordsRemoteIpAddress()
  {
    var dto = await _keyManager.CreateKey(
        tenantId: Guid.NewGuid(),
        creatorId: Guid.NewGuid(),
    ...
    var validateResult = await _keyManager.ValidateKey(dto.KeySecret, dto.Id, deviceId, remoteIp);
  ...
  public async Task ValidateKey_WhenPersistentKeyExists_Succeeds()
  {
    ...
    var validateResult = await _keyManager.ValidateKey(dto.KeySecret);
    Assert.True(validateResult);
  }
  [Fact]
  public async Task ValidateKey_WhenTimeBasedKeyExistsAndExpired_Fails()
  {
    ...
    for (var i = 0; i < 5; i++)
    {
      ...
      validateResult = await _keyManager.ValidateKey(dto.KeySecret);
      Assert.True(validateResult);
    }
  ...
  public async Task ValidateKey_WhenTimeBasedKeyExpired_RemovesKey()
  {
    ...
    validateResult = await _keyManager.ValidateKey(dto.KeySecret, dto.Id);
    Assert.False(validateResult);
  }
  [Theory]
  [InlineData(1)]
  ...
  public async Task ValidateKey_WhenUsageBasedKeyExistsAndUsedUp_Fails(uint allowedUses)
  {
    ...
    var finalValidateResult = await _keyManager.ValidateKey(dto.KeySecret);
  ...
  public async Task ValidateKey_WithIncorrectKeyId_Fails()
  {
    ...
    var validateResult = await _keyManager.ValidateKey(dto.KeySecret, Guid.NewGuid());
    Assert.False(validateResult);
  }
  [Fact]
